...
import { Transaction, TransactionOutputs, BigNum, Value } from "lucid-cardano/custom_modules/cardano-multiplatform-lib-nodejs"
...
const decodeTx = async (tx: Transaction, serverWalletAddr: string) => {
    ...
    const txInputs: UTxO[] = await getFormatedTxInputs(inputs)
...
function getTxOutValues(outputs: TransactionOutputs, serverWalletAddr: string) {
    ...
    for (let outputIndex of [...Array(outputs.len()).keys()]) {
        ...
        const outputAddress = output.address().to_bech32().toString()
...
const getFormatedTxInputs = async (inputs: any) => {
    const txIn: UTxO[] = []
